...
run_containers_module() {
    ...
    if [[ "${INSTALL_PROOT:-1}" -eq 1 ]]; then
        ...
        if command -v proot-distro >/dev/null 2>&1; then
            log_info "proot-distro is present. Target preset: ${PROOT_DISTRO_PRESET:-ubuntu}"
        else
            log_info "proot-distro toolchain will be initialized upon package installation."
        fi
